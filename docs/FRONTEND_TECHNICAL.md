...
> This file covers the **frontend** only. Backend-specific details live in `backend/docs/` (for example `backend/docs/api/README.md`).
...
- **`MainLayout`** (`src/layouts/MainLayout.tsx`)
  ...
  - Renders:
    - `Navbar` (top bar with theme switcher, notifications bell, etc.)
...
- Unit test service classes (ProfileService, NotificationService, etc.).
...
- Use backend docs (`backend/docs/api/README.md`) as the source of truth for contract expectations.
