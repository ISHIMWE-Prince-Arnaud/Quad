      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8 text-center">
          <h1 className="text-4xl font-bold text-gray-900 dark:text-white mb-2 flex items-center justify-center space-x-3">
            <Trophy className="text-accent-400" size={40} />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="space-y-3">
              {data?.topFunny.map((post, index) => (
                <div key={post._id} className="flex items-center space-x-4 p-4 bg-gray-50 dark:bg-gray-800 rounded-lg hover:scale-105 transition-transform">
                  <div className={`text-2xl font-bold ${index === 0 ? 'text-accent-400' : 'text-gray-400'}`}>
                    #{index + 1}
                  </div>
            ...
            <div className="space-y-3">
              {data?.activeUsers.map((user, index) => (
                <div key={user._id} className="flex items-center space-x-4 p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
                  <div className={`text-2xl font-bold ${index < 3 ? 'text-primary-600 dark:text-primary-400' : 'text-gray-400'}`}>
                    #{index + 1}
                  </div>
                  ...
                  <div className="text-right">
                    <div className="text-lg font-bold text-primary-600 dark:text-primary-400">{user.totalReactions}</div>
          ...
          {/* Hall of Fame */}
          <div className="bg-gradient-to-br from-accent-100 to-primary-100 dark:from-accent-900/20 dark:to-primary-900/20 rounded-xl shadow-soft p-6">
            <div className="flex items-center space-x-2 mb-4">
            ...
            <div className="space-y-3">
              {data?.hallOfFame.map((user) => (
                <div key={user._id} className="flex items-center space-x-4 p-4 bg-white/50 dark:bg-dark-card/50 backdrop-blur-sm rounded-lg">
                  <img src={user.avatar} alt={user.username} className="w-12 h-12 rounded-full border-2 border-accent-400" />
                  <div className="flex-1">
                    <p className="font-semibold text-gray-900 dark:text-white">{user.username}</p>
                    <div className="flex flex-wrap gap-1 mt-1">
                      {user.badges?.map((badge) => (
                        <span
                          key={badge}
                          className="text-xs px-2 py-1 bg-accent-400 text-gray-900 rounded-full font-medium"
                        >
                          {badge}
                        </span>
                      ))}
